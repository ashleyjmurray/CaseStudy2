...
##### ashley and nathan look at dis for correlation

(Link to paper about multicollinearity with logistic regression, may be useful: https://www-tandfonline-com.proxy.lib.duke.edu/doi/abs/10.1080/09720502.2010.10700699?journalCode=tjim20)

```{r}
# packages to do all of this
library(broom)
library(car)
library(olsrr)
library(corrplot)
...
full_model <- glm(label ~ ., data = final_na, family = binomial)
```
...
d <- final_na
# apply correlations
corr.d <- cor(d, use="pairwise.complete.obs")
# NAs on the diagonal
...
# get rid of NAs
m <- na.omit(m)
# view the highest correlated: correlation vals > 0.5
(m[ which( m$value > 0.5 ), ])
```

is it true? check *eda_sum_response_time_chest* against *eda_sum_response_areas_chest*

```{r}
modela <- lm(eda_sum_response_time_chest ~ eda_sum_response_areas_chest, data = final)
summary(modela)
```

never seen the warning "essentially perfect fit: summary may be unreliable" b4, iconic
