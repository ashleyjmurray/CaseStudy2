...
pd.DataFrame({"Variable Name":coef_names_ordered, "Coefficient":coef_values_ordered}).to_csv("lasso_logistic_coefs.csv", index = False)
...
pd.DataFrame({"Variable Name":fi_vars_ordered, "Feature Importance":fi_values_ordered}).to_csv("feature_importance_chart.csv", index = False)
...
# Accuracy df
pd.DataFrame({"Accuracy":model_accuracies, "F1 Score":model_f1}).to_csv("model_results.csv", index = False)
